...
mmtp_packet_header_t* mmtp_packet_header_parse_from_block_t(block_t* udp_packet) {
	...
	if(mmtp_packet_header->mmtp_packet_version != 0x1) {
		__MMTP_PARSER_ERROR("MMTP version field != 0x1, value is 0x%x, bailing!", mmtp_packet_header->mmtp_packet_version);
		goto error;
	}
#endif
	...
	if(!(mmtp_packet_header->mmtp_packet_id == 0x0000 && mmtp_packet_header->mmtp_payload_type == 0x2)) {
		if(!(mmtp_packet_header->mmtp_packet_id >= 0x0010 && mmtp_packet_header->mmtp_packet_id <= 0x1FFE)) {
			_MMTP_ERROR("MMTP packet_id is not compliant with A/331 8.1.2.1.3 - MPEG2 conversion compatibility, packet_id: %-10hu (0x%04x)", mmtp_packet_header->mmtp_packet_id, mmtp_packet_header->mmtp_packet_id);
			goto error;
		}
	}
#endif
...
void mmtp_sub_flow_push_mmtp_packet(mmtp_sub_flow_t *mmtp_sub_flow, mmtp_payload_fragments_union_t *mmtp_packet) {
	mmtp_packet_header->mmtp_sub_flow = mmtp_sub_flow;

//	__PRINTF_TRACE("%d:mmtp_sub_flow_push_mmtp_packet, packet_counter: %d, packet_id: %d, mmtp_payload_type: 0x%x\n", __LINE__, mmtp_packet_header->packet_counter, mmtp_packet_header->mmtp_packet_id, mmtp_packet_header->mmtp_payload_type);
	if(mmtp_packet_header->mmtp_payload_type == 0x00) {
		//(mmtp_mpu_type_packet_header_fields_t*, defer, we don;'t know enough about the type

		mpu_fragments_t *mpu_fragments = mpu_fragments_get_or_set_packet_id(mmtp_sub_flow, mmtp_packet_header->mmtp_packet_id);
		atsc3_vector_push(&mpu_fragments->all_mpu_fragments_vector, mmtp_packet);
	} else if(mmtp_packet_header->mmtp_payload_type == 0x01) {
		atsc3_vector_push(&mmtp_sub_flow->mmtp_generic_object_fragments_vector, mmtp_packet);
	} else if(mmtp_packet_header->mmtp_payload_type == 0x02) {
		atsc3_vector_push(&mmtp_sub_flow->mmtp_signalling_message_fragements_vector, mmtp_packet);
	} else if(mmtp_packet_header->mmtp_payload_type == 0x03) {
...
void mmtp_sub_flow_remove_mmtp_packet(mmtp_sub_flow_t *mmtp_sub_flow, mmtp_payload_fragments_union_t *mmtp_packet) {
	...
	if(mmtp_packet_header->mmtp_payload_type == 0x00) {
        ...
        mpu_data_unit_payload_fragments_t* mpu_metadata_fragments_vector = mpu_data_unit_payload_fragments_find_mpu_sequence_number(&mmtp_sub_flow->mpu_fragments->mpu_metadata_fragments_vector, mmtp_mpu_type_packet_header.mpu_sequence_number);
        ...
        mpu_data_unit_payload_fragments_t* movie_metadata_fragments = mpu_data_unit_payload_fragments_find_mpu_sequence_number(&mmtp_sub_flow->mpu_fragments->movie_fragment_metadata_vector, mmtp_mpu_type_packet_header.mpu_sequence_number);
    ...
    //
        mpu_data_unit_payload_fragments_t* data_unit_payload_types = mpu_data_unit_payload_fragments_find_mpu_sequence_number(&mmtp_sub_flow->mpu_fragments->media_fragment_unit_vector, mmtp_mpu_type_packet_header.mpu_sequence_number);
