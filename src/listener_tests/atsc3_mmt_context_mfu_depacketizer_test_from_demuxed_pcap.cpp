/*
 * atsc3_mmt_context_mfu_depacketizer_test_from_demxued_pcap.c
 *
 *  Created on: Sept 13, 2022
 *      Author: jjustman
 *
 * sample extraction for MMT flow(s) to extract MFU emissions for decoder buffer handoff and robustness validation from demuxed pcap
 */
...
#define _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_ERROR(...)    __LIBATSC3_TIMESTAMP_ERROR(__VA_ARGS__);
#define _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_WARN(...)     __LIBATSC3_TIMESTAMP_WARN(__VA_ARGS__);
#define _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_INFO(...)     __LIBATSC3_TIMESTAMP_INFO(__VA_ARGS__);
#define _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_DEBUG(...)    __LIBATSC3_TIMESTAMP_DEBUG(__VA_ARGS__);
#define _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_TRACE(...) // __LIBATSC3_TIMESTAMP_TRACE(__VA_ARGS__);
...
int main(int argc,char **argv) {
    ...
    if(argc == 1) {
            println("%s - DEMUXED pcap replay test harness for atsc3 mmt mfu extraction - output will be in mpu/", argv[0]);
            println("---");
            println("args: demuxed_pcap_file_name (dst_ip) (dst_port) (packet_id)");
            println(" pcap_file_name: pcap demuxed file to process MMT emissions - note: must have both LLS and MMT emission in demuxed flow");
    ...
    if(argc>=4) {
        ...
        _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_DEBUG("reading file %s, dst_ip: %s (%p), dst_port: %s (%p)",
                                                                 PCAP_FILENAME,
                                                                 filter_dst_ip,
                                                                 dst_ip_addr_filter,
                                                                 filter_dst_port,
    ...
    _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_DEBUG("Opening pcap: %s, context is: %p", PCAP_FILENAME, atsc3_pcap_replay_context);
    ...
    if(atsc3_pcap_replay_context) {
        while((atsc3_pcap_replay_context = atsc3_pcap_replay_iterate_packet(atsc3_pcap_replay_context))) {
            ...
            _ATSC3_MMT_CONTEXT_FROM_DEMUXED_PCAP_TEST_DEBUG("pcap reader release: pos: %ld, Got packet len: %d, ts_sec: %u, ts_usec: %u",
