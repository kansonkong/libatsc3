...
void atsc3_preg2_regex_extract_and_assign_named_capture_groups(atsc3_pcre2_regex_context_t *atsc3_pcre2_regex_context, atsc3_preg2_regex_match_capture_group_t *atsc3_preg2_regex_match_capture_group, PCRE2_SPTR subject, PCRE2_SIZE subject_length) {
	pcre2_pattern_info(atsc3_pcre2_regex_context->re, PCRE2_INFO_NAMECOUNT, &atsc3_pcre2_regex_context->namecount);          /* do we have any named capture groups to process?*/

	if (atsc3_pcre2_regex_context->namecount == 0) {
		__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match: atsc3_pcre2_regex_context: %p - No named substrings", atsc3_pcre2_regex_context);
		return;
	}   else {
		PCRE2_SPTR tabptr;
		__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match: atsc3_pcre2_regex_context: %p - have %d namecount captures", atsc3_pcre2_regex_context, atsc3_pcre2_regex_context->namecount);
		
		/* Before we can access the substrings, we must extract the table for
		 translating names to numbers, and the size of each entry in the table. */
		
		(void)pcre2_pattern_info(
								 atsc3_pcre2_regex_context->re,                       /* the compiled pattern */
								 PCRE2_INFO_NAMETABLE,     /* address of the table */
								 &atsc3_pcre2_regex_context->name_table);             /* where to put the answer */
		
		(void)pcre2_pattern_info(
								 atsc3_pcre2_regex_context->re,                       /* the compiled pattern */
								 PCRE2_INFO_NAMEENTRYSIZE, /* size of each entry in the table */
								 &atsc3_pcre2_regex_context->name_entry_size);        /* where to put the answer */
		
		//	Now we can scan the table and, for each entry, print the number, the name,
		//	  and the substring itself. In the 8-bit library the number is held in two
		//	  bytes, most significant first.
		
		tabptr = atsc3_pcre2_regex_context->name_table;
		for (int i = 0; i <  atsc3_pcre2_regex_context->namecount; i++) {
			...
			//use n as the actual pcre2_regex_match capture_id, as our namecount will be sparse if not every capture group is named...
			int n = (tabptr[0] << 8) | tabptr[1];
			
			for(int j = 0; j < atsc3_preg2_regex_match_capture_group->atsc3_preg2_regex_match_capture_v.count && !atsc3_preg2_regex_match_capture; j++) {
				atsc3_preg2_regex_match_capture = atsc3_preg2_regex_match_capture_group->atsc3_preg2_regex_match_capture_v.data[j];
				if(atsc3_preg2_regex_match_capture && atsc3_preg2_regex_match_capture->capture_reference_id != n) {
					atsc3_preg2_regex_match_capture = NULL;
					continue;
				}
								
				//we have a match based upon capture_id, sanity check our match_capture as needed
				//example output looks like:  (1)           		repId: 										  Video1_1
				//							(n) (name_entry_sz) (tabptr+2):		(ovector[2*n+1]-ovector[2*n]) (subject+ovector[2*n] - already captured in atsc3_preg2_regex_match_capture->substring)
				...
				if(atsc3_preg2_regex_match_capture->substring && !strncmp((const char*)(subject + atsc3_pcre2_regex_context->ovector[2*n]), (const char*)block_Get(atsc3_preg2_regex_match_capture->substring), __MIN(substring_length, strlen((const char*)block_Get(atsc3_preg2_regex_match_capture->substring))))) {
					...
					__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: unable to assign named_match: id: %d, atsc3_preg2_regex_match_capture->capture_reference_id: %d, capture_name: %s, capture_val: %s, atsc3_preg2_regex_match_capture->substring: %s",
											  n,
											  atsc3_preg2_regex_match_capture->capture_reference_id,
											  tabptr + 2,
											  subject + atsc3_pcre2_regex_context->ovector[2*n],
											  block_Get(atsc3_preg2_regex_match_capture->substring));
					atsc3_preg2_regex_match_capture = NULL;
					continue;
			...
			//if we get here without an atsc3_preg2_regex_match_capture, then we wern't able match this named capture up
			if(!atsc3_preg2_regex_match_capture) {
				__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: unable to find assigned named_match: id: %d %*s: %.*s, atsc3_preg2_regex_match_capture_group->atsc3_preg2_regex_match_capture_v.count is only: %d!",
										  n,
										  atsc3_pcre2_regex_context->name_entry_size - 3,
										  tabptr + 2,
										  (int)(atsc3_pcre2_regex_context->ovector[2*n+1] - atsc3_pcre2_regex_context->ovector[2*n]),
										  subject + atsc3_pcre2_regex_context->ovector[2*n],
...
atsc3_pcre2_regex_match_capture_vector_t* atsc3_pcre2_regex_match(atsc3_pcre2_regex_context_t* atsc3_pcre2_regex_context, block_t* subject_block) {
	if(!atsc3_pcre2_regex_context || !atsc3_pcre2_regex_context->match_data) {
		__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: atsc3_pcre2_regex_context (or match_data) is null!");
		return NULL;
	}

	if(!subject_block->p_size) {
		__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: subject_block->p_size is 0!");
		return NULL;
	}
	...
	if (rc < 0) {
	  switch(rc) {
	    case PCRE2_ERROR_NOMATCH:
	    	__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match: no match for pattern: %s, with data: %s", atsc3_pcre2_regex_context->pattern, subject_block->p_buffer);
	    	break;
			...
			*/
	    default:
	    		__PCRE2_REGEX_UTILS_INFO("atsc3_pcre2_regex_match: Matching error %d", rc);
	    	break;
	 }
	...
	__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match: Match succeeded at offset %d ", (int)atsc3_pcre2_regex_context->ovector[0]);
	...
	*************************************************************************/
	...
	if (rc == 0) {
		__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: ovector was not big enough for all the captured substrings");
	...
	if (atsc3_pcre2_regex_context->ovector[0] > atsc3_pcre2_regex_context->ovector[1]) {
		__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: \\K was used in an assertion to set the match start after its end."
	...
	__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match: first match capture group dump, rc: %d", rc);
	...
	for (int i = 0; i < rc; i++) {
		...
		__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match:  %2d (s: %zu, e: %zu): %.*s", i,
													  atsc3_pcre2_regex_context->ovector[2*i],
													  atsc3_pcre2_regex_context->ovector[2*i+1],
													  (int)substring_length, (char *)substring_start);
	}
	
	atsc3_preg2_regex_extract_and_assign_named_capture_groups(atsc3_pcre2_regex_context, atsc3_preg2_regex_match_capture_group, subject, subject_length);
	...
	for (;;) {
		...
		if (rc < 0) {
			__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: Matching error %d", rc);
			goto error;
		}

		/* Match succeded */

		__PCRE2_REGEX_UTILS_TRACE("atsc3_pcre2_regex_match: atsc3_pcre2_regex_context: %p, Match succeeded again at offset: %d", atsc3_pcre2_regex_context, (int)atsc3_pcre2_regex_context->ovector[0]);

		/* The match succeeded, but the output vector wasn't big enough. This
		should not happen. */

		if (rc == 0) {
			__PCRE2_REGEX_UTILS_ERROR("atsc3_pcre2_regex_match: atsc3_pcre2_regex_context: %p, ovector was not big enough for all the captured substrings, rc: %d", atsc3_pcre2_regex_context, rc);
		}
...
complete:
	...
	__PCRE2_REGEX_UTILS_DEBUG("atsc3_pcre2_regex_match: returning match capture vector: %p, capture groups: %d", atsc3_pcre2_regex_match_capture_vector, atsc3_pcre2_regex_match_capture_vector->atsc3_preg2_regex_match_capture_group_v.count);
...
void atsc3_preg2_regex_match_capture_free(atsc3_preg2_regex_match_capture_t** atsc3_preg2_regex_match_capture_p) {
	if(atsc3_preg2_regex_match_capture_p) {
		...
		if(atsc3_preg2_regex_match_capture) {
			if(atsc3_preg2_regex_match_capture->capture_name) {
				freeclean((void**)&atsc3_preg2_regex_match_capture->capture_name);
			}
