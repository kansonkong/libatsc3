...
//    error = IT9300_writeRegister (Endeavour, chip, p_br_mp2if_ignore_sync_byte, 0);
	...
	error = IT9300_writeRegister(endeavour, chip, OVA_SMS_ENABLE, 0);
	if (error) goto exit;
...
Dword IT9300_setHighSpeedTSoutClock(
    ...
    if(tsOutClk == HSPEED_CLKOUT_60M)
    {
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_mpclk_out_sel, 0x08);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_top_padmiscdr2, 1);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_top_padmiscdr4, 1);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_top_padmiscdr8, 1);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem17, 0x18);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem17, 0x1B);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem22, 0x89);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem12, 0x14);	//600 / 60 = 10, (10 & 0x7F) * 2 = 20 = 0x14
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem15, 0x8);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem15, 0x28);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem15, 0x8);
		if (error) goto exit;
    }
    else if(tsOutClk == HSPEED_CLKOUT_80M)
    {
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_mpclk_out_sel, 0x08);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_top_padmiscdr2, 1);
		...
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem22, 0x89);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem12, 0x0E);	//600 / 80 = 7, (7 & 0x7F) * 2 = 14 = 0x0E
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem15, 0x8);
		if (error) goto exit;
		error = IT9300_writeRegister(endeavour, chip, p_br_reg_afe_mem15, 0x28);
		...
		error = BR_ERR_INVALID_HSPEED_CLKOUT;
...
Dword IT9300_ExtI2C_write (
	IN  Endeavour*      endeavour,
	IN  Byte            chip,
	IN  Byte            i2cbus,
	IN  Byte            i2cSlaveAddress,
	IN  Byte            wLen,
	IN  Byte*           wBuf,
	IN  Bool            bRepeatStart
) {
	Dword		error = BR_ERR_NO_ERROR;
	Byte		buf[4];
	Byte		index;
	...
	if (wLen > 254)
	{
		return BR_ERR_INVALID_DATA_LENGTH;
	}

	if (bRepeatStart)
	{
	    error = IT9300_writeRegister(endeavour, 0, p_br_reg_repeat_start, 1);
		if (error) goto exit;
	}

	if (wLen <= 48)
	{
		error = IT9300_writeGenericRegisters(endeavour, chip, i2cbus, i2cSlaveAddress, wLen, wBuf);
		if (error) goto exit;
	}
	else
	{
		...
		while (tmpLen >= 48)
		{
			error = IT9300_writeRegisters(endeavour, chip, p_br_reg_tuner_data_15_8 + index, 48, wBuf + index);
			if (error) goto exit;
		...
		if (tmpLen > 0)
		{
			error = IT9300_writeRegisters(endeavour, chip, p_br_reg_tuner_data_15_8 + index, tmpLen, wBuf + index);
			if (error) goto exit;
		}

		buf[0] = 0xF4;
		buf[1] = i2cbus;
		buf[2] = i2cSlaveAddress;
		buf[3] = wLen;
		error = IT9300_writeRegisters(endeavour, chip, 0x4900, 4, buf);
		if (error) goto exit;

		error = IT9300_readRegister(endeavour, chip, OVA_EXTI2C_STATUS, &tmp);
		if (error) goto exit;
		if (tmp != 0)
		{
			error = BR_ERR_EXTI2C_FAIL;
			goto exit;
		}
	}
...
exit:
	if (bRepeatStart)
	    error = IT9300_writeRegister(endeavour, 0, p_br_reg_repeat_start, 0);
	return error;
}


Dword IT9300_ExtI2C_read (
	IN  Endeavour*      endeavour,
	IN  Byte            chip,
	IN  Byte            i2cbus,
	IN  Byte            i2cSlaveAddress,
	...
	if (rLen <= 48)
	{
		error = IT9300_readGenericRegisters(endeavour, chip, i2cbus, i2cSlaveAddress, rLen, rBuf);
		if (error) goto exit;
	}
	else
	{
		...
		while (tmpLen >= 48)
		{
			error = IT9300_readRegisters(endeavour, chip, p_br_reg_tuner_data_7_0 + index, 48, rBuf + index);
			if (error) goto exit;
		...
		if (tmpLen > 0)
		{
			error = IT9300_readRegisters(endeavour, chip, p_br_reg_tuner_data_7_0 + index, tmpLen, rBuf + index);
			if (error) goto exit;
		}
	}
