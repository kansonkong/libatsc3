...
typedef struct {
    ...
    u32		frequency;
    u32		error;
    Byte	mode;//ATSC:0 QAMB:1 QAMC:2 DVBT/T2:3 ISDBT:4 ATSC3:5
    u32		symbolrate;//use for QAMB DVBC
    Byte	reserved[11];
...
/**
 * Get DVB-T2 All MPLP Data.
 * Paramters: MPLPData struct
 */
#define IOCTL_ITE_DVBT2_GETMPLPS \
	_IOR(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_STANDARD + 0x1A, MPLPData)

/**
 * Set an particular DVB-T2 PLP ID.
 * Paramters: MPLPData struct
 */
#define IOCTL_ITE_DVBT2_SETPLPID \
	_IOWR(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_STANDARD + 0x1B, MPLPData)
