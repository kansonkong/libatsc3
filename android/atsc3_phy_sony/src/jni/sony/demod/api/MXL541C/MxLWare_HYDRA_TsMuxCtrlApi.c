...
MXL_STATUS_E MxL_Ctrl_Set_MuxMode_54x(UINT8 devId, MXL_HYDRA_TS_MUX_TYPE_E tsMuxType)
{
	...
	if(MXL_HYDRA_TS_MUX_4_TO_1 == tsMuxType)
	{
		...
		*/
		...
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, MXL_HYDRA_DEMOD_ID_0, (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_0)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, MXL_HYDRA_DEMOD_ID_1, (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_1)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, MXL_HYDRA_DEMOD_ID_2, (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_2)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, MXL_HYDRA_DEMOD_ID_3, (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_3)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_7)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_4)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_7)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_5)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_7)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_6)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_7)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_7)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
	...
	else
	{
		...
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_0)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_0)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_1)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_1)));
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxLWare_HYDRA_CfgXPTXBar(devId, (MXL_HYDRA_DEMOD_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_DEMOD_ID_6)), (MXL_HYDRA_TS_ID_E)(MXL_LOGICAL_TO_PHYSICAL_BYPASS(MXL_HYDRA_TS_ID_6)));
	}
 ...
 *
 * @retval MXL_E_SUCCESS            - OK
 * @retval MXL_E_FAILURE            - Failure
 * @retval MXL_E_INVALID_PARAMETER  - Invalid parameter is passed
 *
 ************************************************************************/
MXL_STATUS_E MxLWare_HYDRA_API_CfgTSMuxMode(UINT8 devId, MXL_HYDRA_TS_MUX_TYPE_E tsMuxType)
{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		{
			mxlStatus = MxL_Ctrl_Set_MuxMode_54x(devId, tsMuxType);
			if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		}
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgTSMixMuxMode(UINT8 devId, MXL_HYDRA_TS_GROUP_E tsGroupType, MXL_HYDRA_TS_MUX_TYPE_E tsMuxType)
{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		{
			mxlStatus = MXL_E_INVALID_PARAMETER;
		}
	}
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgTSPidFilterCtrl(UINT8 devId, MXL_HYDRA_DEMOD_ID_E demodId, MXL_HYDRA_TS_PID_FLT_CTRL_E tsFiltCtrl)
{
	...
	MXL_HYDRA_TS_ID_E		listTsId[MXL_HYDRA_TS_MAX] = { (MXL_HYDRA_TS_ID_E)0, };
...
static MXL_STATUS_E MxL_HYDRA_CfgTSPadMux(UINT8 devId, MXL_HYDRA_DEMOD_ID_E demodId, MXL_HYDRA_MPEG_MODE_E mpegMode)
{
  ...
  MXL_REG_FIELD_T			mxl544_xpt_ts[MXL_HYDRA_DEMOD_MAX][MXL_HYDRA_XPT_TS_MAX] = {
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		{
			devHandlePtr->serialInterface = MXL_E_FALSE;
		...
		{
			devHandlePtr->serialInterface = MXL_E_TRUE;
			switch (devHandlePtr->deviceType)
			{
			default :
			case MXL_HYDRA_DEVICE_561 :
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgMpegOutParams(UINT8 devId, MXL_HYDRA_DEMOD_ID_E demodId, MXL_HYDRA_MPEGOUT_PARAM_T* mpegOutParamPtr)
{
	...
	{
		mxlStatus = MxL_Ctrl_GetTsID(devHandlePtr, demodId, &demodId);
		if ((mxlStatus == MXL_E_SUCCESS) && (mpegOutParamPtr))
		{
			...
			if (MXL_E_ENABLE == mpegOutParamPtr->enable)
			{
				...
				if (mpegOutParamPtr->mpegMode == MXL_HYDRA_MPEG_MODE_PARALLEL)
				{
					...
					mxlStatus = SET_REG_FIELD_DATA(devId, XPT_ENABLE_PARALLEL_OUTPUT, MXL_E_TRUE);
					if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
				}
				else // Serial
				{
					// Disable parallel interface
					mxlStatus = SET_REG_FIELD_DATA(devId, XPT_ENABLE_PARALLEL_OUTPUT, MXL_E_FALSE);
					if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
			...
			if ((MXL_HYDRA_MPEG_MODE_PARALLEL == mpegOutParamPtr->mpegMode) && (MXL_E_DISABLE == mpegOutParamPtr->enable))
			{
...
static SINT16 MxL_HYDRA_GetUnusedLocationIndex(MXL_HYDRA_CONTEXT_T * devHandlePtr, MXL_HYDRA_DEMOD_ID_E demodId)
{
	...
	for (i = 0; i < devHandlePtr->maxRegPidEntries; i++)
	{
		...
		if (pidFilterPtr[i].inUse == MXL_E_FALSE)
		{
			unusedLoc = (SINT16)i;
			break;
		}
                                                      ...
                                                      MXL_HYDRA_TS_PID_T *pidsPtr, UINT8 numPids)
{
	...
	if ((devHandlePtr) && (pidsPtr))
	{
		...
		for (i = 0; i < numPids; i++)
		{
			...
			// input pid
			if ((tmpPidPtr->originalPid < devHandlePtr->maxFixPidEntries) && (devHandlePtr->dss2dvbEnacapMode[demodId] == MXL_E_FALSE))
			{
				...
				// if dirtyflag is MXL_FALSE, then the pid entry will not be updated in the device
				 devHandlePtr->fixedPidFilt[demodId][tmpPidPtr->originalPid].dirtyFlag = MXL_E_TRUE;
				...
				devHandlePtr->fixedPidFilt[demodId][tmpPidPtr->originalPid].data[0] = FIXED_PID_DATA(tmpPidPtr->enable, ((tmpPidPtr->allowOrDrop == MXL_E_TRUE) ? 0 : 1), tmpPidPtr->enablePidRemap, tmpPidPtr->remappedPid);
			...
			else
			{
				...
				default :
				case MXL_HYDRA_TS_MUX_DISABLE :
					startId = (MXL_HYDRA_DEMOD_ID_E)tsId;
					endId = (MXL_HYDRA_DEMOD_ID_E)tsId;
				...
				case MXL_HYDRA_TS_MUX_2_TO_1 :
					...
					{
						startId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_0;
						endId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_1;
					}
					else if ((tsId == MXL_HYDRA_TS_ID_2) || (tsId == MXL_HYDRA_TS_ID_3))
					{
						startId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_2;
						endId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_3;
					}
					else if ((tsId == MXL_HYDRA_TS_ID_4) || (tsId == MXL_HYDRA_TS_ID_5))
					{
						startId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_4;
						endId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_5;
					}
					else
					{
						startId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_6;
						endId = (MXL_HYDRA_DEMOD_ID_E)MXL_HYDRA_TS_ID_7;
					}
					break;
				...
				while((tmpId <= (UINT8)endId) && (regTableIndex == -1))
				{
				...
				if ((regTableIndex >= 0) && (regTableIndex < devHandlePtr->maxRegPidEntries))
				{
					...
					devHandlePtr->regPidFilt[sharedDemodId][regTableIndex].dirtyFlag = MXL_E_TRUE;
					...
					if (devHandlePtr->dss2dvbEnacapMode[demodId] == MXL_E_TRUE)
					{
						pidMask = 0x0FFF;
					}
					else
					...
					devHandlePtr->regPidFilt[sharedDemodId][regTableIndex].data[0] = REGULAR_PID_DATA_0(tmpPidPtr->enable, ((tmpPidPtr->allowOrDrop == MXL_E_TRUE) ? 0 : 1), tmpPidPtr->enablePidRemap, tmpPidPtr->originalPid, pidMask);
				...
				{
					MXL_HYDRA_PRINT("MXL_E_INVALID_PARAMETER: regTableIndex: %d, max regpidentries: %d\n", regTableIndex, devHandlePtr->maxRegPidEntries);
					mxlStatus = MXL_E_INVALID_PARAMETER;
				}
			}
		}
...
static MXL_STATUS_E MxL_HYDRA_CfgUpdatePidFilterTable(UINT8 devId, MXL_HYDRA_CONTEXT_T *devHandlePtr, MXL_HYDRA_TS_ID_E tsId, MXL_BOOL_E updateDirtyFlag)
{
	...
	{
		if (devHandlePtr->fixedPidFilt[tsId][i].dirtyFlag == MXL_E_TRUE)
		{
			...
			// update dirty flag
			if (updateDirtyFlag == MXL_E_TRUE)
			{
				devHandlePtr->fixedPidFilt[tsId][i].dirtyFlag = MXL_E_FALSE;
			}
	    }
	}
	for (sharedFiltId = 0; ((sharedFiltId < devHandlePtr->features.demodsCnt) && (mxlStatus == MXL_E_SUCCESS)); sharedFiltId++)
	{
		...
		if (mxlStatus == MXL_E_SUCCESS)
		{
			...
			for (i = 0; i < devHandlePtr->maxRegPidEntries; i++)
			{
				if (devHandlePtr->regPidFilt[listTsId[1]][i].dirtyFlag == MXL_E_TRUE)
				{
					...
					if (updateDirtyFlag == MXL_E_TRUE)
					{
						devHandlePtr->regPidFilt[listTsId[1]][i].dirtyFlag = MXL_E_FALSE;
					}
				}
			}
		}
                                               ...
                                               UINT8 numPids)
{
	...
	UINT32					pidCount = 0;
	MXL_BOOL_E				cbMode = MXL_E_FALSE;
	MXL_HYDRA_TS_ID_E		tsId = (MXL_HYDRA_TS_ID_E)0;
	MXL_HYDRA_TS_ID_E		listTsId[MXL_HYDRA_TS_MAX] = { (MXL_HYDRA_TS_ID_E)0, };
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		if ((pidsPtr) && (numPids) && ((MXL_E_FALSE == devHandlePtr->features.chanBond) || (pidsPtr[0].bondId < MXL_HYDRA_CB_GROUP_MAX)))
		{
			if (MXL_E_TRUE == devHandlePtr->features.chanBond)
			{
				/* if chan bond is not active then normal pid filter table should be configured */
				if (devHandlePtr->cbBondedGroup[pidsPtr[0].bondId].cbGroupInUse == MXL_E_FALSE)
				{
					cbMode = MXL_E_FALSE;
				}
				else
				...
				if (mxlStatus == MXL_E_SUCCESS)
				{
					for (iter = 0; iter < listTsId[0]; iter++)
					{
						...
						if (mxlStatus == MXL_E_SUCCESS)
						{
							...
							mxlStatus = MxL_HYDRA_CfgUpdatePidFilterTable(devId, devHandlePtr, tsId, MXL_E_FALSE);
							if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
							...
							mxlStatus = MxL_HYDRA_CfgUpdatePidFilterTable(devId, devHandlePtr, tsId, MXL_E_TRUE);
							if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
						}
					} // for (iter = 0;
				}
      ...
      //if (devHandlePtr->features.chanBond == MXL_E_TRUE)
	  if (cbMode == MXL_E_TRUE)
      {
        mxlStatus = MxL_Ctrl_GetTsID(devHandlePtr, demodId, &demodId);
                                               ...
                                               UINT8 *numPidsPtr)
{
  ...
  if ((MXL_E_SUCCESS == mxlStatus) && (pidsInfoPtr) && (numPidsPtr))
  {
  ...
  if (MXL_E_SUCCESS == mxlStatus)
  {
	...
	if (MXL_E_SUCCESS == mxlStatus)
    {
      ...
      for(tsIndex = 0; (tsIndex < tsCount) && (pidCount < maxNumPids); tsIndex++)
      {
        ...
        {
			if ((mxlStatus == MXL_E_SUCCESS) && ((regReadData[loopCount] & 0x1) == 0x1) && ((UINT32)tsId == ((regReadData[loopCount + 1] >> 16) & 0x07)))
          {
			...
			pidsInfoPtr[pidCount].enable = MXL_E_TRUE;
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgTsMuxPrefixExtraTsHeader(UINT8 devId, MXL_HYDRA_DEMOD_ID_E demodId, MXL_HYDRA_TS_MUX_PREFIX_HEADER_T * prefixHeaderPtr)
{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		mxlStatus = MxL_Ctrl_GetPhyTsID(devId, demodId, listTsId);
		if ((mxlStatus == MXL_E_SUCCESS) && (prefixHeaderPtr))
		{
			for (i = 0; i < listTsId[0] && i < MXL_HYDRA_TS_MAX; i++)
			{
				...
				// enable TS header
				if (prefixHeaderPtr->enable == MXL_E_TRUE)
				{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		if (mxlStatus == MXL_E_SUCCESS)
		{
			...
			mxlStatus = MxLWare_Hydra_UpdateByMnemonic(devId,
												  ...
												  (MXL_E_TRUE)); // invert DSS enable or disable
			if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
			...
			if (enableDssEncapsulation == MXL_E_TRUE)
			{
                                                          ...
                                                          MXL_BOOL_E enable)
{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		mxlStatus = MxLWare_Hydra_UpdateByMnemonic(devId,
											...
											(enable == MXL_E_TRUE) ? (mxlExtTSInputId + 1) : MXL_E_FALSE);
		if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
	}
