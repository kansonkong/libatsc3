...
#include "MxLWare_HYDRA_FW_Download.h"
...
static MXL_BOOL_E MxL_Ctrl_IsFirmwareAlive(UINT8 devId, UINT16 timeoutMs)
{
	...
	UINT32				regData = 0;
	MXL_BOOL_E			fwActiveFlag = MXL_E_FALSE;
          ...
          (fwActiveFlag == MXL_E_FALSE))
	{
		...
		if ((fwHeartBeat) && (regData) && (!mxlStatus))
		{
			...
			{
				fwActiveFlag = MXL_E_TRUE;
			}
		}

/*
    MXLDBG2(
      if (fwActiveFlag == MXL_E_FALSE)
      {
...
*/
	...
	MXLDBG2(MXL_HYDRA_PRINT("Firmware %s after %d attempts\n", (fwActiveFlag == MXL_E_TRUE)?"running":"dead", attempt););
	MXLERR(if (mxlStatus != MXL_E_SUCCESS) MXL_HYDRA_ERROR("Error reading register\n"););
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		if (prcmChipId != 0x560)
		{
			...
			case MXL_HYDRA_SKU_ID_581 :
				skuOK = ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_581) || (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_581S))?MXL_E_TRUE:MXL_E_FALSE;
				if(skuOK == MXL_E_TRUE)
				{
					devHandlePtr->deviceType = MXL_HYDRA_DEVICE_581;
				}
				MXLDBG1(MXL_HYDRA_PRINT("SKU selection MxL581 %s\n", (skuOK == MXL_E_TRUE) ? "correct" : "incorrect"););
				break;

			case MXL_HYDRA_SKU_ID_584 :
				skuOK = (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_584)?MXL_E_TRUE:MXL_E_FALSE;
				MXLDBG1(MXL_HYDRA_PRINT("SKU selection MxL584 %s\n", (skuOK == MXL_E_TRUE) ? "correct" : "incorrect"););
				break;

			case MXL_HYDRA_SKU_ID_585 :
				skuOK = (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_585)?MXL_E_TRUE:MXL_E_FALSE;
				MXLDBG1(MXL_HYDRA_PRINT("SKU selection MxL585 %s\n", (skuOK == MXL_E_TRUE) ? "correct" : "incorrect"););
				break;

			case MXL_HYDRA_SKU_ID_544 :
				skuOK = ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_544) || (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_542))?MXL_E_TRUE:MXL_E_FALSE;
			...
			case MXL_HYDRA_SKU_ID_582 :
				skuOK = (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_582)?MXL_E_TRUE:MXL_E_FALSE;
		...
		else // Scorpius/Ceres PRCM CHIP ID == 0x560
		{
			if ((prcmSoCId == 0x4) && ((padMuxBond & 0x8) == 0x0)) // Scorpius
			{
				...
				case MXL_HYDRA_SKU_ID_544 :
					skuOK = ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_541S) || (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_541) || 
							 (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_541C))?MXL_E_TRUE:MXL_E_FALSE;
					if (skuOK == MXL_E_TRUE)
					{
				...
				case MXL_HYDRA_SKU_ID_581 :
					skuOK = ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_581S) || (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_581) || 
							 (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_581C))?MXL_E_TRUE:MXL_E_FALSE;
					if (skuOK == MXL_E_TRUE)
					{
				...
				case MXL_HYDRA_SKU_ID_561 :
					// Sku = 561S
					skuOK = ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_561S) || (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_561)) ? MXL_E_TRUE : MXL_E_FALSE;
					if (skuOK == MXL_E_TRUE)
					{
				...
				case MXL_HYDRA_SKU_ID_568 :
					...
					skuOK = (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_568) ? MXL_E_TRUE : MXL_E_FALSE;
			...
			else if ((prcmSoCId == 0x5) && ((padMuxBond & 0x8) == 0x8)) // Ceres
			{
				...
				case MXL_HYDRA_SKU_ID_582 :
					skuOK = (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_582C) ? MXL_E_TRUE : MXL_E_FALSE;
					if (skuOK == MXL_E_TRUE)
					{
				...
				case MXL_HYDRA_SKU_ID_584 :
					skuOK = (devHandlePtr->deviceType == MXL_HYDRA_DEVICE_542C) ? MXL_E_TRUE : MXL_E_FALSE;
					if (skuOK == MXL_E_TRUE)
					{
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDevXtal(UINT8 devId, MXL_HYDRA_DEV_XTAL_T * xtalParamPtr, MXL_BOOL_E enableClkOut)
{
	...
	if (status == MXL_E_SUCCESS)
	{
		...
		if ((devHandlePtr->features.clkOutAvailable == MXL_E_FALSE) && (enableClkOut == MXL_E_ENABLE))
		{
		...
		else
		{
			...
			if (status != MXL_E_SUCCESS) return status;
...
{
	MXL_HYDRA_CONTEXT_T*	devHandlePtr;
	MXL_STATUS_E			mxlStatus = MXL_E_FAILURE;
	UINT32					regData = 0;
	MXL_HYDRA_SKU_COMMAND_T devSkuCfg;
	//UINT8					cmdSize = sizeof(MXL_HYDRA_SKU_COMMAND_T);
	UINT8					cmdSize = MXL_HYDRA_SKU_COMMAND_T_SIZE;
							...
							if (MXL_E_SUCCESS == mxlStatus)
							{
								...
								if (MXL_E_SUCCESS == mxlStatus)
								{
									...
									if (MXL_E_SUCCESS == mxlStatus)
									{
										...
										if ((MXL_E_TRUE == MxL_Ctrl_IsFirmwareAlive(devId, 750)) && (mxlStatus == MXL_E_SUCCESS))
										{
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDevInterrupt(UINT8 devId, MXL_HYDRA_INTR_CFG_T intrCfg, UINT32 intrMask)
{
	...
	UINT8					cmdSize = MXL_INTR_CFG_T_SIZE;
	UINT8					cmdBuff[MXL_HYDRA_OEM_MAX_CMD_BUFF_LEN];
...
MXL_STATUS_E MxLWare_HYDRA_API_ReqDevVersionInfo(UINT8 devId, MXL_HYDRA_VER_INFO_T *versionInfoPtr)
{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		if (versionInfoPtr)
		{
			...
			if (versionInfoPtr->firmwareDownloaded == MXL_E_TRUE)
			{
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDevPowerMode(UINT8 devId, MXL_HYDRA_PWR_MODE_E pwrMode)
{
	...
	UINT8					cmdSize = MxL_HYDRA_POWER_MODE_CMD_SIZE;
	UINT8					cmdBuff[MXL_HYDRA_OEM_MAX_CMD_BUFF_LEN];
	...
	if (status == MXL_E_SUCCESS)
	{
		devPowerCfg.powerMode = (UINT8)pwrMode;
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDevWakeOnRF(UINT8 devId, MXL_HYDRA_TUNER_ID_E tunerId, UINT32 wakeTimeInSeconds, SINT32 rssiThreshold)
{
	...
	UINT8					cmdSize = MXL_HYDRA_RF_WAKEUP_CFG_T_SIZE;
	UINT8					cmdBuff[MXL_HYDRA_OEM_MAX_CMD_BUFF_LEN];
                              ...
                              UINT8 * matchBufferPtr, UINT8 * maskBufferPtr)
{
	...
	UINT8					cmdSize = MXL_HYDRA_BROADCAST_WAKEUP_CFG_T_SIZE;
	UINT8					cmdBuff[MXL_HYDRA_OEM_MAX_CMD_BUFF_LEN];


	interruptLockFail = MXL_E_FALSE;
	...
	if ((status == MXL_E_SUCCESS) && (chanTuneParamsPtr) && (awakeTimeInSeconds >= MXL_HYDRA_MIN_TIME_IN_SECS && awakeTimeInSeconds <= MXL_HYDRA_MAX_TIME_IN_SECS) &&
	...
	{
		MXL_HYDRA_TUNER_ID_E tunerId = (MXL_HYDRA_TUNER_ID_E)0;
		MXL_HYDRA_DEMOD_ID_E demodId = (MXL_HYDRA_DEMOD_ID_E)0;
		...
		broadcastWakeCfg.interruptLockFail = (UINT32)(interruptLockFail == MXL_E_TRUE) ? 1 : 0;
...
MXL_STATUS_E MxLWare_HYDRA_API_CfgDevOverwriteDefault(UINT8 devId)
{
	...
	if (status == MXL_E_SUCCESS)
	{
		if (MxL_Ctrl_ValidateSku(devHandlePtr) == MXL_E_TRUE)
		{
...
MXL_STATUS_E  MxLWare_HYDRA_API_CfgDevDiseqcFskOpMode(UINT8 devId, MXL_HYDRA_AUX_CTRL_MODE_E devAuxCtrlMode)
{
  ...
  if (status == MXL_E_SUCCESS)
  {
    ...
    {
		devHandlePtr->features.FSKEnabled = MXL_E_TRUE;
		devHandlePtr->features.DiSEqCEnabled = MXL_E_FALSE;
    }
    else
    {
      devHandlePtr->features.FSKEnabled = MXL_E_FALSE;
	  devHandlePtr->features.DiSEqCEnabled = MXL_E_TRUE;
    }
  }
...
MXL_STATUS_E  MxLWare_HYDRA_API_CfgDevGPOData(UINT8 devId, UINT8 gpioId, MXL_HYDRA_GPIO_DATA_E gpioData)
{
  ...
  if (mxlStatus == MXL_E_SUCCESS)
  {
    if ((gpioId < devHandlePtr->gpioMaxNum) && (gpioData < MXL_HYDRA_GPIO_DATA_MAX))
    {
	  ...
	  if ((mxlStatus == MXL_E_SUCCESS) && (regReadValue == MXL_HYDRA_GPIO_DIR_OUTPUT))
      {
        mxlStatus = MxLWare_Hydra_UpdateByMnemonic(devId, 
