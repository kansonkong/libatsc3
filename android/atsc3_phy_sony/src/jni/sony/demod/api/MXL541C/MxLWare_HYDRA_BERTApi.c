                                           ...
                                           MXL_BOOL_E invertOutputPolarity)
{
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
      ...
      mxlStatus |= MxLWare_Hydra_UpdateByMnemonic(devId, xpt_reg_addr, xpt_lsb_pos, xpt_width, 1);
			...
			xpt_lsb_pos += (((UINT8) tsId) * xpt_width);
			mxlStatus = MxLWare_Hydra_UpdateByMnemonic(devId, xpt_reg_addr, xpt_lsb_pos, xpt_width, (invertBertData == MXL_E_TRUE)?1:0);
			if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;

			// XPT_BERT_INVERT_SEQUENCEx
			MxLWare_HYDRA_ExtractFromMnemonic(XPT_BERT_INVERT_SEQUENCE0, &xpt_reg_addr, &xpt_lsb_pos, &xpt_width);
			xpt_lsb_pos += (((UINT8) tsId) * xpt_width);
			mxlStatus = MxLWare_Hydra_UpdateByMnemonic(devId, xpt_reg_addr, xpt_lsb_pos, xpt_width, (invertSequencePolarity == MXL_E_TRUE) ? 1 : 0);
			if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;

			// XPT_BERT_OUTPUT_POLARITYx
			MxLWare_HYDRA_ExtractFromMnemonic(XPT_BERT_OUTPUT_POLARITY0, &xpt_reg_addr, &xpt_lsb_pos, &xpt_width);
			xpt_lsb_pos += (((UINT8) tsId) * xpt_width);
			mxlStatus = MxLWare_Hydra_UpdateByMnemonic(devId, xpt_reg_addr, xpt_lsb_pos, xpt_width, (invertOutputPolarity == MXL_E_TRUE) ? 0 : 1);
			if (mxlStatus != MXL_E_SUCCESS) return mxlStatus;
		}
		else
		{
                                                 ...
                                                 MXL_BOOL_E *berkLockStatusPtr)
{
	...
	{
		mxlStatus = MxL_Ctrl_GetTsID(devHandlePtr, demodId, &demodId);
		if ((mxlStatus == MXL_E_SUCCESS) && (berkLockStatusPtr))
		{
			tsId = (MXL_HYDRA_TS_OUT_ID_E)demodId;
			if ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_544) && (devHandlePtr->mpegMode[demodId] == MXL_HYDRA_MPEG_MODE_SERIAL_4_WIRE))
			{
			...
			if (mxlStatus == MXL_E_SUCCESS)
			{
				*berkLockStatusPtr = (regData == 1) ? MXL_E_TRUE : MXL_E_FALSE;
			}
		}
		else
		{
                                            ...
                                            MXL_BOOL_E *bitCountSaturationFlagPtr)
{
	...
	{
		mxlStatus = MxL_Ctrl_GetTsID(devHandlePtr, demodId, &demodId);
		if ((mxlStatus == MXL_E_SUCCESS) && (totalbitCountPtr) && (totalbitErrorPtr) && (bitCountSaturationFlagPtr))
		{
			tsId = (MXL_HYDRA_TS_OUT_ID_E)demodId;
			if ((devHandlePtr->deviceType == MXL_HYDRA_DEVICE_544) && (devHandlePtr->mpegMode[demodId] == MXL_HYDRA_MPEG_MODE_SERIAL_4_WIRE))
			{
			...
			*bitCountSaturationFlagPtr = (regData) ? MXL_E_TRUE : MXL_E_FALSE;
