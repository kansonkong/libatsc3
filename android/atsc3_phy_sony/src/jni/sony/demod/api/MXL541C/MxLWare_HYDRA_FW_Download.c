                                               ...
                                               /*@null@*/ MXL_CALLBACK_FN_T fwCallbackFn)
{
	...
	UINT32			currentSize = MBIN_FILE_HEADER_T_SIZE;


	cbPayload.totalSize = mbinBufferSize;
	//currentSize += sizeof(MBIN_SEGMENT_HEADER_T);
	currentSize += MBIN_SEGMENT_HEADER_T_SIZE;
	...
	do
	{
		...
		MxL_CovertDataForEndianness(MXL_E_TRUE, size, wBufPtr);
		...
		if (fwCallbackFn)
		{
			...
			if ((fwCBFreq >= (UINT32)(7 * blockSize)) || (cbPayload.currentSize == cbPayload.totalSize))
			{
				if (MXL_HYDRA_FW_DOWNLOAD_ABORT == fwCallbackFn(devId, (UINT32)fwCBFreq, &cbPayload))
				{
					...
					mxlStatus = MXL_E_NOT_READY; // FW ABORT request received
					break;
				}
  ...
  UINT32 data = 0;
	...
	if (mxlStatus == MXL_E_SUCCESS)
	{
		...
		if (mbinPtr->header.id == MBIN_FILE_HEADER_ID)
		{
			...
			for (index = 0; index < mbinPtr->header.numSegments; index++)
			{
				...
				if ( segmentPtr->header.id != MBIN_SEGMENT_HEADER_ID)
				{
					...
					mxlStatus = MXL_E_NOT_SUPPORTED; // If the segment format is wrong, then exit with MXL_FAILURE
					break;
				}
		...
		else
		{
			...
			mxlStatus = MXL_E_NOT_SUPPORTED; // the mbin file formate not supported
		}
	}
